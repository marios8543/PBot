...
class Vote():
	...
	def __init__(self,ctx,question,options,duration):
...
async def vote(ctx):
	...
	embed.set_author(name="Vote by {}".format(vote.user.name+'#'+vote.user.discriminator))
	if vote.duration>60:
		duration = str(vote.duration//60)+' minutes remaining'
	else:
		duration = str(vote.duration)+' seconds remaining'
	...
	vote.embed = msg
	votes_running[msg.id] = vote
	#Timer implementation (rewrite pending)
	elapsed = 0
	while vote.duration>=elapsed:
	...
	winner = vote.get_winner()
	if winner==6:
		embed=discord.Embed(title='The answer is a draw!!!')
	else:	
		embed=discord.Embed(title='The answer is '+vote.options[winner]['option'])	
	embed.set_author(name=vote.question)
	for option in vote.options:
		embed.add_field(name=option['option'],value=str(len(option['votes']))+' votes')
	embed.set_footer(text='Vote called by '+vote.user.name+'#'+vote.user.discriminator)
